...
def init(style_file):
    ...
    class UI:
        ...
        class Subtitle(UIAsset):
            ...
            def draw(self, sc=None):
                if self.render:
                    self.texture = subtitle_fnt.render(
                        self.text, 1, subtitle_fg)
                    ...
                    self.rect = Rect(
                        self.pos.x, self.pos.y, self.texture.get_width(), self.texture.get_height())
                    self.render = False
                if sc:
            ...
            def draw(self, sc=None):
                if self.render:
                    ...
                    if self.spos.y == 2:
                        ...
                        self.pos.y -= self.rpos.y
                    self.pos += self.parent.get_pos(sc)
                    self.rect = Rect(self.pos.x+margin, self.pos.y+margin,
                                     self.size.x-margin*2, self.size.y-margin*2)
                    self.render = False
                if self.border:
                    if sc:
        ...
        class RadioList(UIAsset):
            ...
            def draw(self, sc=None):
                if self.render:
                    ...
                    draw.rect(self.texture, border_fg,
                              (0, 0, *self.size), border_width)
                    em = radiolist_fnt.get_height()
                    dpos = Vector2(0, 0)
                    self.rects = []
                    for n, i in enumerate(self.items):
                        r = Rect(*(dpos+Vector2(radiolist_margin, radiolist_margin)+Vector2(
                            0, radiolist_margin)*(2*n-1)), self.size.x-radiolist_margin*2, em)
                        r.x += self.pos.x
                        r.y += self.pos.y
                        self.rects.append(r)
                        if n == self.hover:
                            draw.rect(self.texture, radiolist_hover_bg, (*(dpos+Vector2(radiolist_margin, radiolist_margin)+Vector2(
                                0, radiolist_margin)*(2*n-1)), self.size.x-radiolist_margin*2, em))
                        if n == self.active:
                            draw.rect(self.texture, radiolist_active_bg, (*(dpos+Vector2(radiolist_margin, radiolist_margin)+Vector2(
                                0, radiolist_margin)*(2*n-1)), self.size.x-radiolist_margin*2, em))
        ...
        class Window(UIAsset):
            def __init__(self, size=Vector2(800, 600), flags=RESIZABLE, name="", icon=""):
                ...
                self.surf = display.set_mode(
                    (int(size.x), int(size.y)), HWACCEL | HWPALETTE | HWSURFACE | DOUBLEBUF | flags)
