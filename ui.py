...
class Subtitle(UIAsset):
    ...
    def draw(self, sc=None):
        if self.render:
            ...
            self.rect = Rect(self.pos.x, self.pos.y,
    ...
    def draw(self, sc=None):
        if self.render:
            self.texture = Surface(
                (int(self.size.x), int(self.size.y)), SRCALPHA)
    ...
    def draw(self, sc=None):
        if self.render:
            ...
            for n, i in enumerate(self.items):
                r = Rect(*(dpos+Vector2(radiolist_margin, radiolist_margin)+Vector2(0,
                         radiolist_margin)*(2*n-1)), self.size.x-radiolist_margin*2, em)
                r.x += self.pos.x
                r.y += self.pos.y
                self.rects.append(r)
                if n == self.hover:
                    draw.rect(self.texture, radiolist_hover_bg, (*(dpos+Vector2(radiolist_margin, radiolist_margin) +
                              Vector2(0, radiolist_margin)*(2*n-1)), self.size.x-radiolist_margin*2, em))
                if n == self.active:
                    draw.rect(self.texture, radiolist_active_bg, (*(dpos+Vector2(radiolist_margin, radiolist_margin) +
                              Vector2(0, radiolist_margin)*(2*n-1)), self.size.x-radiolist_margin*2, em))
